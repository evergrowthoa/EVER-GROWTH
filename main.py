...
def run_script():
    try:
        ...
        col_note = get_col_index_by_headers(headers1, "특이사항")
        col_contract = get_col_index_by_headers(headers1, "계약번호")
        ...
        for idx, row1 in df1[condition].iterrows():
            ...
            status2 = str(match_row.get("상태", "")).strip()
            if status2 not in ["신용조사(가완료)", "신용조사", "출고의뢰"]:
                log_rows.append([
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    customer_name,
                    contract_no,
            ...
            m_val = str(match_row.get("배정시간", "")).strip()
            ...
            a1_note = rowcol_to_a1(rownum, col_note)
            ...
            value_updates.append({
                "range": f"{ws1.title}!{a1_note}",
                "values": [[combined_note]]
            })

            format_requests.append(color_request(ws1.id, rownum, col_status, 1, 1, 0.6))
            format_requests.append(color_request(ws1.id, rownum, col_note, 1, 1, 0.6))
